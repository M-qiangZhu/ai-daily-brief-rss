...
- 支持 RSS/Atom、HTML 列表页、GitHub Releases、Hugging Face API、百度新闻搜索和 IMAP 邮箱新闻。
- 官方资讯优先保留，英文标题和摘要保持原文。
- 支持 RSS/Atom 数据源，包括阮一峰博客、IT之家、V2EX、新智元等。
- 使用关键词过滤，只保留人工智能、模型、Agent、AI 编程、算力芯片、机器人、AI 硬件、AI 安全等相关内容，并过滤普通消费电子终端噪声。
- 每天按中国时间自然日生成当日资讯，并永久保存每日 JSON 归档。
- 同时生成今日页面、历史归档页面、归档索引、前端 JSON 数据和本地 latest 数据。
- 补跑同一天时会用最新抓取结果刷新同链接/同标题的旧归档字段，便于分类规则修正后更新本地和服务器页面。
...
2. 并发请求启用的 RSS/Atom、HTML、GitHub Releases、Hugging Face、百度新闻和 IMAP 邮箱来源。
...
7. 更新 `docs/archive/index.json` 归档索引和最新一天的前端兼容数据；同一天补跑时，新抓取结果会覆盖旧归档中的同链接/同标题记录。
8. 使用官方、媒体、社区分层配额，避免社区或聚合媒体挤占官方动态。
9. 同一天内按领导关注优先级排序：政策、运营商、算力芯片、AI 技术、AI 模型、AI Agent、机器人、AI 硬件、安全监管、AI 编程、一般 AI 资讯。
10. 前端会进一步按领导视角聚合为：政策与监管、南通市官方 AI 动态、运营商与央国企动态、算力/数据中心/云基础设施、AI 技术与产业应用、投融资与竞争格局、风险安全与合规、技术社区观察。

分类规则会优先判断新闻主体，避免邮箱长摘要中的背景词误导分类：

- 政策与监管需要标题或正文出现明确监管动作，例如标准、办法、通知、行动方案、数据安全、征集起草等；不会因为摘要里泛泛出现“政策”或“补贴政策”就归为政策。
- AI 硬件、机器人/世界模型、AI Agent/模型优先于泛化的算力词；例如 AI 眼镜归入 AI 硬件，世界模型/机器人归入 AI 技术与产业应用。
- 算力、数据中心与云基础设施只用于真正围绕算力芯片、GPU/NPU、数据中心、云基础设施、边缘 AI 系统或网络承载能力的新闻。
...
部署示例使用 `deploy/ai-daily-brief.cron`，每天 `00:00、08:50、16:00` 抓取最近 48 小时内容并合并到每日归档，用于补偿晚间发布、源站延迟和临时网络失败。

企业微信摘要固定在每天 `09:00` 发送一次，直接读取 `08:50` 推送前检索生成的当日归档，不会额外触发检索。信源异常仅保留在后台日志中，不写入企业微信摘要。
...
命令执行时还会生成企业微信机器人可用的 markdown 摘要。设置 `WECHAT_WEBHOOK_URL` 并添加 `--notify` 后，会把公网页面链接和当天领导摘要推送到企业微信群。测试渠道可使用 `WECHAT_WEBHOOK_URL_TEST`，该渠道会自动在消息开头加测试备注；正式渠道和测试渠道的正文保持一致。

RSS/Atom 来源会读取 feed 条目的标题、链接、发布时间和摘要/正文片段。百度新闻来源会访问搜索结果页，从结果块中提取标题、链接、来源、摘要和中文时间。IMAP 邮箱来源会只读拉取当天邮件，优先解析 HTML 正文中的新闻列表、表格行、标题块、摘要块和原文链接；纯文本邮件会回退解析“标题 + 链接”格式。
...
- macOS、iOS、手机、平板、耳机、智能穿戴、车机、系统更新、发热、电池、售价、参数等普通消费电子终端内容默认过滤；只有明确涉及端侧模型、本地推理、AI 芯片/算力、机器人、企业 AI 平台或重要 AI 生态战略时才保留。
...
- 行业方向：机器人、具身智能、AI SaaS、AI 搜索、AI 安全、AI 监管等；汽车和自动驾驶类新闻仅在明确涉及算力、芯片或 AI 基础设施时保留。
...
- `feeds`：RSS/Atom 来源。
- `searches`：百度新闻搜索来源。
- `sources`：统一信源注册表，可配置 `feed`、`html`、`github_atom`、`huggingface_api`、`baidu_news`、`imap_mail` 等来源。
...
邮箱账号和授权码不要写入 `data/sources.json`。在服务器 `.env` 中配置：

```bash
MAIL_NEWS_IMAP_USER="your_account@163.com"
MAIL_NEWS_IMAP_PASSWORD="your_client_authorization_code"
MAIL_NEWS_IMAP_HOST="imap.163.com"
MAIL_NEWS_IMAP_PORT="993"
```

163 邮箱需要先在网页端开启 IMAP/SMTP 服务，并生成客户端授权码。程序使用 `BODY.PEEK[]` 读取邮件，不删除、不移动、不标记已读。

如果邮件 HTML 使用“新闻标题 + 摘要 + 来源 + 查看原文”的表格或列表结构，解析器会优先取标题块作为新闻标题，而不是把“查看原文”当标题。
...
本项目约定：本地测试通过后，再推送 GitHub 或部署服务器。
...
这会执行 `tests` 目录中的单元测试，覆盖 RSS 解析、AI 关键词过滤、消费电子终端噪声过滤、百度新闻结果解析、IMAP 邮件 HTML/纯文本解析、分类主体识别、归档刷新和页面数据写入。
