...
- 使用关键词过滤，只保留人工智能、模型、Agent、AI 编程、算力芯片、机器人、AI 硬件、AI 安全等相关内容。
...
RSS/Atom 来源会读取 feed 条目的标题、链接、发布时间和摘要/正文片段。百度新闻来源会访问搜索结果页，从结果块中提取标题、链接、来源、摘要和中文时间。IMAP 邮箱来源会只读拉取当天邮件，优先解析 HTML 正文中的新闻链接，纯文本邮件会回退解析“标题 + 链接”格式。
...
这会执行 `tests` 目录中的单元测试，覆盖 RSS 解析、AI 关键词过滤、百度新闻结果解析、IMAP 邮件 HTML/纯文本解析和页面数据写入。
