...
部署示例使用 `deploy/ai-daily-brief.cron`，每 6 小时执行一次。每次抓取最近 48 小时并合并到每日归档，用于补偿晚间发布、源站延迟和临时网络失败。

企业微信摘要每天只发送一次。后续 6 小时周期任务仍会更新页面和归档，但不会重复推送当天通知。信源异常仅保留在页面状态和后台日志中，不写入企业微信摘要。
